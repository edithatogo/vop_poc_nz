...
def print_summary(stats_list: List[Dict]):
    ...
    # Calculate averages
    avg_words = sum(s['word_count'] for s in stats_list) / len(stats_list) if stats_list else 0
    avg_lines = sum(s['line_count'] for s in stats_list) / len(stats_list) if stats_list else 0
    avg_word_len = sum(s['avg_word_length'] for s in stats_list) / len(stats_list) if stats_list else 0
    avg_sentence_len = sum(s['avg_sentence_length'] for s in stats_list) / len(stats_list) if stats_list else 0
    avg_syllables = sum(s['avg_syllables_per_word'] for s in stats_list) / len(stats_list) if stats_list else 0
    avg_complexity = sum(s['complexity_score'] for s in stats_list) / len(stats_list) if stats_list else 0
    ...
    print("-" * 100)
    print(f"{'File':<30} {'Lines':<8} {'Words':<8} {'Avg Word':<10} {'Avg Sent':<10} {'Complexity':<12} {'Syllables/W':<12}")
    print("-" * 100)
    ...
    for stat in stats_list:
        ...
        print(f"{filename:<30} {line_count:<8} {word_count:<8} {avg_word_len:<10.2f} {avg_sentence_len:<10.2f} {complexity_score:<12.2f} {syllables_per_word:<12.2f}")
    ...
    if stats_list:
        ...
        least_complex = min(stats_list, key=lambda x: x['complexity_score'] if x['complexity_score'] > 0 else float('inf'))

        print(f"\nMost complex file: {os.path.basename(most_complex['filename'])} (score: {most_complex['complexity_score']})")
        print(f"Least complex file: {os.path.basename(least_complex['filename'])} (score: {least_complex['complexity_score']})")

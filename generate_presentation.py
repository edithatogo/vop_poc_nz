...
VERSION = "v3"
OUTPUT_FILE = os.path.join(OUTPUT_DIR, f"Health_Economic_Analysis_{VERSION}_{DATE_STR}.pptx")
...
def create_presentation():
    ...
    def add_table_slide(csv_path, slide_title):
        ...
        header = rows[0]
        target_cols = ["Intervention", "Perspective", "ICER ($/QALY)", "Cost-Effective (WTP=$50k)"]
    ...
    for img in trees:
        name = os.path.basename(img).replace("decision_tree_", "").replace(".png", "").replace("_", " ").title()
        add_image_slide(img, f"Model Structure: {name}", f"Decision tree structure for {name} intervention.")

    # CE Planes
    planes = sorted(glob.glob(os.path.join(FIGURES_DIR, "ce_plane_*.png")))
    for img in planes:
        name = os.path.basename(img).replace("ce_plane_", "").replace(".png", "").replace("_", " ").title()
        add_image_slide(img, f"Cost-Effectiveness Plane: {name}", "Scatter plot of incremental costs vs incremental QALYs.")

    # Comparative ICER Table
    add_table_slide(os.path.join(DATA_DIR, "comparative_icer_table.csv"), "Comparative ICER Summary")

    # Probabilistic Sensitivity Analysis (CEAC/CEAF)
    ceac = os.path.join(FIGURES_DIR, "probabilistic_analysis_ceac_ceaf.png")
    add_image_slide(ceac, "Probabilistic Sensitivity Analysis", "Cost-Effectiveness Acceptability Curve and Frontier.")

    # Equity Analysis
    equity = os.path.join(FIGURES_DIR, "equity_analysis.png")
    add_image_slide(equity, "Equity Analysis", "Distributional impact of interventions across socioeconomic groups.")
    
    dcea_wtp = os.path.join(FIGURES_DIR, "dcea_willingness_to_pay.png")
    add_image_slide(dcea_wtp, "DCEA: Willingness to Pay", "Willingness to pay for equity-weighted health gains.")
    ...
    add_image_slide(bia, "Budget Impact Analysis", "Financial implications of the interventions over time.")

    # VOI
    evpi = os.path.join(FIGURES_DIR, "voi_analysis_evpi.png")
    add_image_slide(evpi, "Value of Information: EVPI", "Expected Value of Perfect Information analysis.")

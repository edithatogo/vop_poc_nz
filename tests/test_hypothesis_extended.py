...
def test_icer_scaling_invariance(cost, qalys, multiplier):
    ...
    # Skip edge cases where floating point underflow/overflow could occur
    assume(cost * multiplier > 1e-300)
    assume(qalys * multiplier > 1e-300)
...
@given(
    n=st.integers(min_value=5, max_value=20),
    wtp=st.floats(
        min_value=1000, max_value=200000, allow_nan=False, allow_infinity=False
    ),
)
...
@given(
    cost=st.floats(min_value=0, max_value=1e6, allow_nan=False, allow_infinity=False),
    qalys=st.floats(
        min_value=0.01, max_value=50, allow_nan=False, allow_infinity=False
    ),
...
@given(
    ...
    inc_qalys=st.floats(
        min_value=0.001, max_value=50, allow_nan=False, allow_infinity=False
    ),
)
...
@given(
    ...
    rate=st.floats(
        min_value=0.01, max_value=0.15, allow_nan=False, allow_infinity=False
    ),

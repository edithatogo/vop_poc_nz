...
class TestValueOfInformation(unittest.TestCase):
    def setUp(self):
        ...
        self.psa_results = pd.DataFrame({
            ...
            "inc_qalys": np.random.normal(2, 0.5, n),
    ...
    def test_psa_run(self):
        ...
        self.assertIn("inc_cost", results.columns)
        self.assertIn("nmb", results.columns)
    ...
    def test_calculate_value_of_perspective(self):
        psa_hs = self.psa_results.copy()
        psa_soc = self.psa_results.copy()
        psa_soc["inc_cost"] = psa_hs["inc_cost"] * 1.1
        ...
        self.assertIn("evpi_given_perspective", vop)

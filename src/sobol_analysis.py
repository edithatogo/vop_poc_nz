...
class SobolAnalyzer:
    ...
    def saltelli_sampling(self) -> Tuple[pd.DataFrame, pd.DataFrame, List[pd.DataFrame]]:
        """
    ...
    def calculate_sobol_indices(self) -> Dict[str, pd.DataFrame]:
        ...
        indices_df = pd.DataFrame({
            ...
            'interaction': [total - first for total, first in zip(total_order, first_order)]
        })
...
def plot_sobol_indices(
    ...
    ax.barh(y_pos, indices_df['total_order'], height=0.4,
           label='Total-order ($S_{Ti}$)', color='steelblue', alpha=0.7)
    
    # Plot first-order indices (main effects only)
    ax.barh(y_pos, indices_df['first_order'], height=0.4,
           label='First-order ($S_i$)', color='darkblue', alpha=0.9)
    ...
    for i, (param, interaction) in enumerate(zip(params, indices_df['interaction'])):
        if interaction > 0.05:  # Only annotate significant interactions
            ax.annotate(f'Interaction: {interaction:.2f}',

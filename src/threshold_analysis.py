...
def run_threshold_analysis(intervention_name: str, base_params: Dict, parameter_ranges: Dict, wtp_threshold: float = 50000) -> Dict:
    """
    ...
    for param_name, param_range in parameter_ranges.items():
        ...
        for param_value in param_range:
            ...
            if 'new_treatment_cost_multiplier' in param_name:
                test_params['costs']['health_system']['new_treatment'][0] = base_params['costs']['health_system']['new_treatment'][0] * param_value
                test_params['costs']['societal']['new_treatment'][0] = base_params['costs']['societal']['new_treatment'][0] * param_value
            elif 'new_treatment_qaly_multiplier' in param_name:
                test_params['qalys']['new_treatment'][0] = base_params['qalys']['new_treatment'][0] * param_value
                test_params['qalys']['new_treatment'][1] = base_params['qalys']['new_treatment'][1] * param_value
            # Add more specific parameter modification logic as needed
            
            # Run analysis
            hs_result = run_cea(test_params, perspective='health_system', wtp_threshold=wtp_threshold)
            soc_result = run_cea(test_params, perspective='societal', wtp_threshold=wtp_threshold)

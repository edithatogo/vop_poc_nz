...
def plot_comparative_cash_flow(
    ...
    for idx, (intervention_name, bia_df) in enumerate(bia_results.items()):
        ...
        ax.plot(years, net_costs, 'o-', linewidth=2.5, markersize=8,
               label=intervention_name, color=colors[idx], alpha=0.8)
    
    ax.axhline(y=0, color='black', linestyle='--', linewidth=1, alpha=0.5)
    ax.set_xlabel('Year', fontsize=12)
    ax.set_ylabel(ylabel, fontsize=12)
    ax.set_title('Comparative Budget Impact Analysis\nAll Interventions', fontsize=14, fontweight='bold')
...
def plot_icer_ladder(
    ...
    for name, results in intervention_results.items():
        if perspective in results:
            if isinstance(results[perspective], dict) and 'incremental_cost' in results[perspective]:
    ...
    inc_qalys_sorted, inc_costs_sorted, interventions_sorted, icers_sorted = zip(*sorted_data)
    
    # Plot points
    colors = plt.cm.viridis(np.linspace(0, 1, len(interventions)))
    for i, (qaly, cost, name, icer) in enumerate(zip(inc_qalys_sorted, inc_costs_sorted, interventions_sorted, icers_sorted)):
        ax.scatter(qaly, cost, s=300, color=colors[i], alpha=0.7, edgecolors='black', linewidth=2, zorder=3)
        ax.annotate(name, (qaly, cost), xytext=(10, 10), textcoords='offset points',
                   fontsize=9, bbox=dict(boxstyle='round,pad=0.5', facecolor=colors[i], alpha=0.3))
    
    # Connect efficient frontier
    ax.plot(inc_qalys_sorted, inc_costs_sorted, 'k--', linewidth=1.5, alpha=0.5, zorder=2, label='Frontier')
    
    # WTP threshold line
    max_qaly = max(inc_qalys_sorted) if inc_qalys_sorted else 1
    ax.plot([0, max_qaly], [0, max_qaly * wtp_threshold], 'r--', linewidth=2, 
           alpha=0.6, label=f'WTP = ${wtp_threshold:,}/QALY')
    ...
    ax.set_title(f'ICER Ladder: {perspective.replace("_", " ").title()} Perspective', fontsize=14, fontweight='bold')
...
def plot_nmb_comparison(
    ...
    for intervention_name in interventions:
        ...
        for persp in perspectives:
            if persp in results:
                if isinstance(results[persp], dict) and 'incremental_nmb' in results[persp]:
    ...
    for i, persp in enumerate(perspectives):
        ...
        bars = ax.bar(x + offset, nmb_data[persp], width, 
                     label=persp.replace('_', ' ').title(),
                     color=colors.get(persp, f'C{i}'), alpha=0.8,
    ...
    ax.set_title('Comparative Net Monetary Benefit\nAcross Interventions and Perspectives', 
...
def plot_equity_impact_comparison(
        ...
        if 'societal' in results:
            ...
            if isinstance(societal, dict):
                for method in ['human_capital', 'friction_cost']:
                    if method in societal and 'dcea_equity_analysis' in societal[method]:
    ...
    if not interventions:
        ...
        fig.text(0.5, 0.5, 'No equity analysis data available',
    ...
    ax1.bar(x - 0.2, atkinson_indices, width=0.4, label='Atkinson Index', 
           color='coral', alpha=0.8, edgecolor='black')
    ax1.bar(x + 0.2, gini_coefficients, width=0.4, label='Gini Coefficient',
           color='skyblue', alpha=0.8, edgecolor='black')
    ax1.set_xlabel('Intervention', fontsize=11)
    ax1.set_ylabel('Inequality Index', fontsize=11)
    ax1.set_title('Inequality Measures\n(Lower = More Equitable)', fontsize=12, fontweight='bold')
    ax1.set_xticks(x)
    ...
    colors = plt.cm.viridis(np.linspace(0, 1, len(interventions)))
    bars = ax2.bar(x, weighted_nmb, color=colors, alpha=0.8, edgecolor='black', linewidth=1.5)
    ax2.set_xlabel('Intervention', fontsize=11)
    ax2.set_ylabel('Equity-Weighted Total Health Gain ($)', fontsize=11)
    ax2.set_title('Equity-Weighted Net Monetary Benefit\n(Higher = Better)', fontsize=12, fontweight='bold')
    ax2.set_xticks(x)
...
def plot_comprehensive_intervention_summary(
    ...
    for idx, (name, bia_df) in enumerate(bia_results.items()):
        ...
        ax1.plot(years, net_costs, 'o-', linewidth=2, markersize=6,
    ...
    bars = ax2.barh(interventions, nmb_values, color=colors_nmb, alpha=0.7, edgecolor='black')
    ...
    for i, (name, qaly, cost) in enumerate(zip(names, inc_qalys, inc_costs)):
        ax3.scatter(qaly, cost, s=200, color=scatter_colors[i], alpha=0.7, 
                   edgecolors='black', linewidth=2, label=name)
    
    max_qaly = max(inc_qalys) if inc_qalys else 1
    ax3.plot([0, max_qaly], [0, max_qaly * wtp_threshold], 'r--', 
            linewidth=2, alpha=0.6, label=f'WTP=${wtp_threshold:,}')
    ...
    for name in interventions:
        ...
        icer_str = f"${icer:,.0f}" if icer not in [np.inf, -np.inf] else "Dominated" if icer == np.inf else "Dominant"
    ...
    plt.suptitle('Comprehensive Intervention Comparison Dashboard', 

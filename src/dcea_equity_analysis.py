...
def calculate_atkinson_index(net_health_benefits: List[float], epsilon: float = 0.5) -> float:
    ...
    if epsilon == 1:
        ...
        return 1 - ((1 / len(array)) * np.sum(array**(1 - epsilon)))**(1 / (1 - epsilon)) / np.mean(array)
...
def generate_dcea_results_table(dcea_results: Dict, intervention_name: str) -> pd.DataFrame:
    """
    ...
    for subgroup, nhb_value in dcea_results['distribution_of_net_health_benefits'].items():
        rows.append({
            'Intervention': intervention_name,
...
def plot_lorenz_curve(dcea_results: Dict, intervention_name: str, output_dir: str = "output/figures/"):
    """
    ...
    nhb_list = sorted(list(dcea_results['distribution_of_net_health_benefits'].values()))
    n = len(nhb_list)
    if n == 0:
    ...
    nhb_list_positive = [max(0, x) for x in nhb_list] # Take max(0,x) to handle negative NHB

    if np.sum(nhb_list_positive) == 0: # If all NHB are zero or negative
        print(f"Cannot plot Lorenz curve for {intervention_name} as all net health benefits are zero or negative after adjustment.")
        return
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/lorenz_curve_{intervention_name.lower().replace(' ', '_')}.png", bbox_inches="tight")
    plt.savefig(f"{output_dir}/lorenz_curve_{intervention_name.lower().replace(' ', '_')}.pdf", bbox_inches="tight")
    plt.close()


def plot_equity_impact_plane(dcea_results: Dict, intervention_name: str, output_dir: str = "output/figures/"):
    """
    ...
    # Annotate quadrants
    ax.text(0.95, 0.95, 'Win-Win', transform=ax.transAxes, ha='right', va='top', fontsize=12, color='green')
    ax.text(0.05, 0.05, 'Lose-Lose', transform=ax.transAxes, ha='left', va='bottom', fontsize=12, color='red')
    ax.text(0.95, 0.05, 'Efficiency Gain, Equity Loss', transform=ax.transAxes, ha='right', va='bottom', fontsize=12)
    ax.text(0.05, 0.95, 'Equity Gain, Efficiency Loss', transform=ax.transAxes, ha='left', va='top', fontsize=12)

    plt.tight_layout()
    plt.savefig(f"{output_dir}/equity_impact_plane_{intervention_name.lower().replace(' ', '_')}.png", bbox_inches="tight")
    plt.savefig(f"{output_dir}/equity_impact_plane_{intervention_name.lower().replace(' ', '_')}.pdf", bbox_inches="tight")
    plt.close()
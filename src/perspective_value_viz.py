...
def plot_perspective_value_dashboard(
    ...
    plt.suptitle(
        f"Value of Perspective Analysis: {intervention_name}\nWTP = ${wtp:,}/QALY",
        fontsize=16,
        fontweight="bold",
        y=0.98,
    )
...
def generate_perspective_value_table(
    ...
    df["Proportion Discordant"] = df["Proportion Discordant"].apply(
        lambda x: f"{x:.1%}"
    )
...
def plot_perspective_comparison_scatter(
    ...
    # Plot points
    ax.scatter(
        nmb_hs[concordant_adopt],
        nmb_soc[concordant_adopt],
        c="green",
        alpha=0.5,
        s=20,
        label="Concordant Adopt",
        edgecolors="none",
    )
    ax.scatter(
        nmb_hs[concordant_reject],
        nmb_soc[concordant_reject],
        c="red",
        alpha=0.5,
        s=20,
        label="Concordant Reject",
        edgecolors="none",
    )
    ax.scatter(
        nmb_hs[discordant],
    ...
    ax.set_ylabel("NMB: Societal Perspective ($)", fontsize=12)
    ax.set_title(
        f"Perspective Agreement Analysis: {intervention_name}\nWTP = ${wtp_threshold:,}/QALY",
        fontsize=14,
        fontweight="bold",
    )

...
class MarkovModel:
    ...
    def run(self, 
            ...
            costs: np.ndarray, 
            qalys: np.ndarray) -> Tuple[float, float]:
        """
...
def _validate_model_parameters(params: Dict):
    ...
    required_keys = ['states', 'transition_matrices', 'cycles', 'initial_population', 'costs', 'qalys']
    for key in required_keys:
        if key not in params:
            raise ValueError(f"Missing required parameter: {key}")
    
    # Validate structure of transition_matrices
    if 'standard_care' not in params['transition_matrices'] or 'new_treatment' not in params['transition_matrices']:
        raise ValueError("transition_matrices must contain 'standard_care' and 'new_treatment' keys")
    ...
    for perspective_type in ['health_system', 'societal']:
        if 'standard_care' not in params['costs'][perspective_type] or 'new_treatment' not in params['costs'][perspective_type]:
            raise ValueError(f"costs[{perspective_type}] must contain 'standard_care' and 'new_treatment' keys")
...
def _get_costs_qalys_by_perspective(model_parameters: Dict, perspective: str, productivity_cost_method: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    if perspective == 'health_system':
        ...
        costs_new = np.array(model_parameters['costs']['health_system']['new_treatment'], dtype=float)
...
def _calculate_icer(inc_cost: float, inc_qalys: float) -> Union[float, str]:
    ...
    if abs(icer) > 1e10:  # Very large ICER
        warnings.warn(f"Extremely large ICER calculated: {icer}. Verify inputs.")
...
def create_parameters_table(model_parameters: Dict, sources: Dict = None) -> pd.DataFrame:
    ...
    rows.append({
        ...
        'Source': sources.get('cycles', 'Model specification') if sources else 'Model specification',
        'Perspective': 'All'
    })
    ...
    rows.append({
        ...
        'Source': sources.get('discount_rate', 'Standard practice') if sources else 'Standard practice',
        'Perspective': 'All'
    })
    ...
    for i, state in enumerate(model_parameters['states']):
        for j, to_state in enumerate(model_parameters['states']):
            rows.append({
                ...
                'Value': model_parameters['transition_matrices']['standard_care'][i][j],
                'Description': f'Annual probability of transitioning from {state} to {to_state} under standard care',
                'Source': sources.get(f'transition_{state}_{to_state}_standard', 'Model specification') if sources else 'Model specification',
                'Perspective': 'All'
            })
            
            rows.append({
                'Parameter': f'Transition {state}→{to_state} (New Treatment)',
                'Value': model_parameters['transition_matrices']['new_treatment'][i][j],
                'Description': f'Annual probability of transitioning from {state} to {to_state} with new treatment',
                'Source': sources.get(f'transition_{state}_{to_state}_new', 'Model specification') if sources else 'Model specification',
                'Perspective': 'All'
            })
    ...
    for i, state in enumerate(model_parameters['states']):
        ...
        rows.append({
            ...
            'Source': sources.get(f'cost_hs_{state}_standard', 'Literature') if sources else 'Literature',
        ...
        rows.append({
            ...
            'Source': sources.get(f'cost_hs_{state}_new', 'Literature') if sources else 'Literature',
            'Perspective': 'Health System'
        })
        ...
        rows.append({
            ...
            'Source': sources.get(f'cost_societal_{state}_standard', 'Literature') if sources else 'Literature',
        ...
        rows.append({
            ...
            'Source': sources.get(f'cost_societal_{state}_new', 'Literature') if sources else 'Literature',
            'Perspective': 'Societal'
        })
    ...
    for i, state in enumerate(model_parameters['states']):
        rows.append({
            ...
            'Source': sources.get(f'qaly_{state}_standard', 'Literature') if sources else 'Literature',
            'Perspective': 'All'
        })
        ...
        rows.append({
            ...
            'Source': sources.get(f'qaly_{state}_new', 'Literature') if sources else 'Literature',
            'Perspective': 'All'
        })
...
def generate_comparative_icer_table(results_health_system: Dict, 
                                  results_societal: Dict, 
                                  intervention_name: str = "Intervention") -> pd.DataFrame:
    """
    ...
    data = {
        ...
        'ICER ($/QALY)': [
            results_health_system['icer'],
            results_societal['icer']
        ],
...
if __name__ == '__main__':
    ...
    comparison_table = generate_comparative_icer_table(hs_results, s_results, "HPV Vaccination")

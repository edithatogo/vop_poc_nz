...
def generate_core_tables(params, output_dir):
    ...
    table_1_data = [
        ...
        {"Item": "Perspectives", "Description": "Health System (Base Case), Societal (Scenario)"},
        {"Item": "Target Population", "Description": "Varies by intervention (see Table 22)"},
        {"Item": "Time Horizon", "Description": "Lifetime (CEA), 5 Years (BIA)"},
        {"Item": "Discount Rate", "Description": f"{discount_rate:.1%} per annum (Costs and QALYs)"},
    ...
    for name, p in params.items():
        if name in ['new_high_cost_cancer_drug', 'smoking_cessation_counselling', 'smoking_cessation_structural_sensitivity']: continue
        rows_2.append({
    ...
    for name, p in params.items():
        ...
        if total_pop > 0:
            dist = [f"{s}: {n/total_pop:.1%}" for s, n in zip(states, init_pop) if n > 0]
            rows_3.append({
                "Intervention": name.replace('_', ' ').title(),
                "Baseline State Distribution": ", ".join(dist)
            })
    tables['Table_3_Baseline_Epidemiology'] = dataframe_to_markdown(pd.DataFrame(rows_3))
    ...
    tables['Table_7_Health_State_Utilities'] = dataframe_to_markdown(pd.DataFrame(rows_7))
    ...
    if dsa_params:
        ...
        tables['Table_11_Parameter_Distributions'] = dataframe_to_markdown(pd.DataFrame(rows_11))
        tables['Table_12_DSA_Specifications'] = dataframe_to_markdown(pd.DataFrame(rows_11)) # Similar content
...
def generate_qualitative_tables(params, output_dir):
    ...
    rows_9 = [
        {"Assumption": "Adherence", "Rationale": "100% adherence assumed for initial treatment phase (simplification)."},
        {"Assumption": "Waning Effect", "Rationale": "Treatment effect constant over time unless specified otherwise."},
        {"Assumption": "General Mortality", "Rationale": "Based on NZ Life Tables 2020-2022."},
...
def generate_cea_tables(results, output_dir):
    ...
    for name, res in results['intervention_results'].items():
        for perspective in ['health_system', 'societal']:
            if perspective in res:
                data = res[perspective].get('human_capital', res[perspective]) if perspective == 'societal' else res[perspective]
                ...
                rows.append({
                    ...
                    "ICER": "Dominant" if data['incremental_cost'] < 0 and data['incremental_qalys'] > 0 else f"${data['icer']:,.0f}" if data['incremental_qalys'] > 0 else "Dominated",
        ...
        for name, dsa_res in results['dsa_analysis']['1_way'].items():
            for perspective in ['health_system', 'societal']:
                if perspective in dsa_res:
                    ...
                    sorted_params = sorted(res_p.items(), key=lambda x: x[1]['range'], reverse=True)[:5]
    ...
    if dsa_rows:
        tables['Table_13_Key_DSA_Results'] = dataframe_to_markdown(pd.DataFrame(dsa_rows))
            ...
            if 'distribution_of_net_health_benefits' in res:
                ...
                for group, nhb in dist.items():
                    rows_15.append({
                        ...
                        "Cost-Effective?": "Likely (Positive NHB)" if nhb > 0 else "Unlikely"
                    })

    if rows_15:
        tables['Table_15_Subgroup_Results'] = dataframe_to_markdown(pd.DataFrame(rows_15))
...
def generate_dcea_tables(results, output_dir):
    ...
    if dcea_rows:
        tables['Table_19_DCEA_Outcomes_by_Group'] = dataframe_to_markdown(pd.DataFrame(dcea_rows))
    ...
    if metrics_rows:
        tables['Table_20_Equity_Impact_Metrics'] = dataframe_to_markdown(pd.DataFrame(metrics_rows))
...
def generate_extended_dcea_tables(results, params, output_dir):
    ...
    if rows_16:
        tables['Table_16_Equity_Population'] = dataframe_to_markdown(pd.DataFrame(rows_16))
    ...
    if rows_17:
        tables['Table_17_Group_Parameters'] = dataframe_to_markdown(pd.DataFrame(rows_17))

    # Table 18: Baseline Distribution
    # Placeholder as we don't have easy access to baseline-only runs in results
    tables['Table_18_Baseline_Distribution'] = "See DCEA Report for detailed baseline distributions."
...
def generate_bia_tables(results, output_dir):
    ...
    if bia_rows:
        tables['Table_24_Annual_Budget_Impact'] = dataframe_to_markdown(pd.DataFrame(bia_rows))
    ...
    if breakdown_rows:
        tables['Table_25_Budget_Impact_Breakdown'] = dataframe_to_markdown(pd.DataFrame(breakdown_rows))
...
def generate_extended_bia_tables(results, params, output_dir):
    ...
    tables['Table_22_BIA_Target_Population'] = dataframe_to_markdown(pd.DataFrame(rows_22))
    ...
    rows_23 = [
        ...
        {"Item": "Included Costs", "Description": "Drug/Intervention Acquisition, Administration, Monitoring"},
    ]
    tables['Table_23_BIA_Perspective'] = dataframe_to_markdown(pd.DataFrame(rows_23))

    # Table 26: BIA Scenarios (Placeholder)
    tables['Table_26_BIA_Scenarios'] = "Scenario analysis results available in full BIA report."
...
def generate_voi_tables(results, output_dir):
    ...
    if evppi_rows:
        tables['Table_28_EVPPI_Summary'] = dataframe_to_markdown(pd.DataFrame(evppi_rows))
...
def generate_extended_voi_tables(results, output_dir):
    ...
    tables['Table_29_EVSI'] = "EVSI analysis requires specific trial design parameters and was not conducted for this summary."

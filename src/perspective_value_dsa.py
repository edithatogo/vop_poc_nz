...
def perform_perspective_value_dsa(
    ...
    # Define PSA wrapper
    def psa_run_cea_wrapper(sampled_params, intervention_type, base_params=model_params):
    ...
    for i, wtp in enumerate(wtp_thresholds):
        ...
        vop = calculate_value_of_perspective(
            psa_results_hs, psa_results_soc, wtp_threshold=wtp, chosen_perspective="health_system"
        )
...
def plot_perspective_value_dsa(
    ...
    line1 = ax1.plot(wtp / 1000, dsa_results["evp"], 'o-', linewidth=2.5, markersize=6,
                    color='steelblue', label='EVP (Opportunity Loss)', alpha=0.8)
    line2 = ax1_twin.plot(wtp / 1000, dsa_results["perspective_premium"], 's-',
                         linewidth=2.5, markersize=6, color='forestgreen',
                         label='Perspective Premium', alpha=0.8)
    ...
    ax1.set_title('Expected Value of Perspective & Premium', fontsize=12, fontweight='bold')
    ...
    ax2 = axes[0, 1]
    ax2.plot(wtp / 1000, dsa_results["discordance_cost"], 'o-', linewidth=2.5,
            markersize=6, color='coral', label='Discordance Cost', alpha=0.8)
    ax2.plot(wtp / 1000, dsa_results["information_value"], 's-', linewidth=2.5,
            markersize=6, color='purple', label='Information Value', alpha=0.8)
    ...
    ax3 = axes[1, 0]
    ax3.plot(wtp / 1000, np.array(dsa_results["proportion_discordant"]) * 100,
            'o-', linewidth=2.5, markersize=6, color='red',
            label='% Discordant Decisions', alpha=0.8)
    ax3.plot(wtp / 1000, np.array(dsa_results["prob_hs_optimal"]) * 100,
            's-', linewidth=2.5, markersize=6, color='steelblue',
            label='% HS Perspective Optimal', alpha=0.8)
    ax3.plot(wtp / 1000, np.array(dsa_results["prob_soc_optimal"]) * 100,
            '^-', linewidth=2.5, markersize=6, color='forestgreen',
            label='% Societal Optimal', alpha=0.8)
    ...
    ax4 = axes[1, 1]
    ax4.plot(wtp / 1000, dsa_results["correlation"], 'o-', linewidth=2.5,
            markersize=6, color='darkblue', alpha=0.8)
    ax4.axhline(y=1.0, color='green', linestyle='--', alpha=0.5, label='Perfect Correlation')
    ...
    plt.suptitle(f'Perspective Value Sensitivity Analysis: {intervention_name}\n'

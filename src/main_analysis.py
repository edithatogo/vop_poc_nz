        ...
        "costs": {
            "health_system": {
                ...
                "new_treatment": [
                    ...
                    400,
                    1500,
                ],  # Program cost + reduced complication costs
            },
            "societal": {
                "standard_care": [
                    0,
                    1000,
                    3000,
                ],  # Lost out-of-pocket, informal care etc.
        ...
        "transition_matrices": {
            "standard_care": [
                ...
                [
                    ...
                    0,
                    1,
                ],  # Complications -> Complications (absorbing state for simplicity)
            ],
            "new_treatment": [
...
def get_housing_insulation_parameters() -> Dict:  # pragma: no cover - demo parameters
    """
        ...
        "costs": {
            "health_system": {
                ...
                "new_treatment": [
                    ...
                    500,
                    1000,
                ],  # Program cost + reduced illness costs
            },
            "societal": {
                "standard_care": [
                    0,
...
def run_corrected_analysis():  # noqa: C901  # pragma: no cover - demonstration pipeline
    """
    ...
    for name, params in selected_interventions.items():
        ...
        for method in ["human_capital", "friction_cost"]:
            ...
            s_results = run_cea(
                params, perspective="societal", productivity_cost_method=method
            )
            ...
            comp_table = generate_comparative_icer_table(
                hs_results, s_results, table_name
            )
    ...
    print(
        f"  Estimated cost for implementing societal perspective: ${capacity_costs['total_cost']:,.0f}"
    )
    ...
    integrated_results = integrate_dce_with_cea(
        dcea_results, all_results["HPV Vaccination"]["societal"]["human_capital"]
    )
    ...
    for name, params in selected_interventions.items():
        ...
        def psa_run_cea_wrapper(sampled_params, intervention_type, base_params=params):
            temp_params = copy.deepcopy(base_params)  # Start with base parameters
            ...
            if (
                ...
                and "new_treatment" in temp_params["qalys"]
            ...
            if (
                ...
                and "new_treatment" in temp_params["transition_matrices"]
            ...
            if intervention_type == "standard_care":
                return cea_results_dict["cost_standard_care"], cea_results_dict[
                    "qalys_standard_care"
                ]
            elif intervention_type == "new_treatment":
                return cea_results_dict["cost_new_treatment"], cea_results_dict[
                    "qalys_new_treatment"
                ]
            else:
                raise ValueError(
                    "Invalid intervention_type provided to psa_run_cea_wrapper"
                )
    ...
    final_results = {
        ...
        "dcea_equity_analysis": {
            name: res.get("dcea_equity_analysis") for name, res in all_results.items()
        },
    }
...
def perform_voi_analysis(hpv_params: Dict) -> Dict:
    ...
    psa = ProbabilisticSensitivityAnalysis(
        simple_model, psa_params, wtp_threshold=50000
    )
...
def run_empirical_dcea_if_available() -> dict:  # pragma: no cover - optional file-based flow
    ...
    cost_attr = None
    coeff_items = list(coeffs)
    for a in coeff_items:
        if "cost" in a.lower() or "price" in a.lower():
            cost_attr = a
            break
    ...
    dce_results = {
        ...
        "preference_heterogeneity": {
            "demographic_segments": [
                ...
                if c.startswith("demo_")
                or c in ["stakeholder_type", "age_group", "gender"]
            ],

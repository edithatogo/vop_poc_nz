...
def perform_one_way_dsa(models, wtp_threshold=50000, n_points=20):
    ...
    for model_name, model_params in models.items():
        ...
        # Calculate base NMB
        base_hs_results = run_cea(model_params, perspective='health_system', wtp_threshold=wtp_threshold)
        base_nmb_hs = (base_hs_results['incremental_qalys'] * wtp_threshold) - base_hs_results['incremental_cost']
        base_soc_results = run_cea(model_params, perspective='societal', wtp_threshold=wtp_threshold)
        base_nmb_soc = (base_soc_results['incremental_qalys'] * wtp_threshold) - base_soc_results['incremental_cost']
        ...
        for param_name, param_range in param_ranges.items():
            ...
            for param_value in param_range:
                ...
                hs_results = run_cea(temp_params, perspective='health_system', wtp_threshold=wtp_threshold)
                nmb_hs = (hs_results['incremental_qalys'] * wtp_threshold) - hs_results['incremental_cost']
                param_results_hs.append(nmb_hs)

                # Societal perspective
                soc_results = run_cea(temp_params, perspective='societal', wtp_threshold=wtp_threshold)
                nmb_soc = (soc_results['incremental_qalys'] * wtp_threshold) - soc_results['incremental_cost']
...
def plot_one_way_dsa_tornado(dsa_results, output_dir="data/data_outputs/figures/"):
    ...
    for model_name, results in dsa_results.items():
        for perspective in ['hs', 'soc']:
            ...
            sorted_indices = np.argsort([np.max(r) - np.min(r) for r in nmb_ranges])[::-1]
            ...
            for i, nmb_range in enumerate(nmb_ranges):
                ...
                ax.barh(y_pos[i], max_nmb - min_nmb, left=min_nmb, color='skyblue', alpha=0.7)

            ax.axvline(base_nmb, color='black', linestyle='--')
            ax.set_yticks(y_pos)
            ax.set_yticklabels(param_names)
            ax.set_xlabel("Net Monetary Benefit ($)")
            ax.set_title(f"One-Way DSA Tornado Plot: {model_name} ({perspective.upper()})")
            
            plt.tight_layout()
            plt.savefig(f"{output_dir}/one_way_dsa_tornado_{model_name.lower().replace(' ', '_')}_{perspective}.png", bbox_inches='tight')
            plt.close()
    ...
    for model_name, model_params in models.items():
        ...
        for p1 in param1_range:
            for p2 in param2_range:
                ...
                hs_results = run_cea(temp_params, perspective='health_system', wtp_threshold=wtp_threshold)
                nmb_hs = (hs_results['incremental_qalys'] * wtp_threshold) - hs_results['incremental_cost']
                dsa_grid_hs.append({"param1": p1, "param2": p2, "nmb": nmb_hs, "icer": hs_results['icer']})

                # Societal perspective
                soc_results = run_cea(temp_params, perspective='societal', wtp_threshold=wtp_threshold)
                nmb_soc = (soc_results['incremental_qalys'] * wtp_threshold) - soc_results['incremental_cost']
                dsa_grid_soc.append({"param1": p1, "param2": p2, "nmb": nmb_soc, "icer": soc_results['icer']})
...
def plot_two_way_dsa_heatmaps(dsa_results, output_dir="data/data_outputs/figures/"):
    ...
    for perspective in ['hs', 'soc']:
        ...
        if n_models == 1:
            axes = [axes]
        fig.suptitle(f"Two-Way Deterministic Sensitivity Analysis: NMB at $50,000/QALY\n{'Health System' if perspective == 'hs' else 'Societal'} Perspective", fontsize=14, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/two_way_dsa_heatmaps_{perspective}.png", bbox_inches='tight')
        plt.savefig(f"{output_dir}/two_way_dsa_heatmaps_{perspective}.pdf", bbox_inches='tight')
        plt.close()
            ...
            for p2 in p2_range:
                for p3 in p3_range:
                    ...
                    if "HPV" in model_name:
                        ...
                        temp_params['transition_matrices']['new_treatment'][0][0] = 1 - temp_params['transition_matrices']['new_treatment'][0][1] - temp_params['transition_matrices']['new_treatment'][0][2]
                    elif "Hepatitis C" in model_name:
                        temp_params['costs']['health_system']['new_treatment'][0] *= p1
                        temp_params['qalys']['new_treatment'][1] *= p2
                    ...
                    soc_results = run_cea(temp_params, perspective='societal', wtp_threshold=wtp_threshold)
                    nmb = (soc_results['incremental_qalys'] * wtp_threshold) - soc_results['incremental_cost']
                    dsa_grid.append({"p1": p1, "p2": p2, "p3": p3, "nmb": nmb, "icer": soc_results['icer']})
...
def plot_three_way_dsa_3d(dsa_results, output_dir="data/data_outputs/figures/"):
    ...
    for model_name in model_names:
        ...
        # Create 3D grid
        p1_grid, p2_grid, p3_grid = np.meshgrid(data['p1_range'], data['p2_range'], data['p3_range'], indexing='ij')

        # Interpolate NMB values onto the grid
        points = np.array([[point['p1'], point['p2'], point['p3']] for point in data['dsa_grid']])
        ...
        fig.suptitle(f"Three-Way Deterministic Sensitivity Analysis: {model_name}\nNMB at $50,000/QALY (Societal Perspective)", fontsize=14, fontweight='bold')
        ...
        for i, p3_slice in enumerate(p3_slices):
            ...
            if np.sum(mask) > 10:  # Only plot if we have enough points
                ...
                if len(p1_unique) > 1 and len(p2_unique) > 1:
                    ...
                    grid_nmb = griddata((slice_points[:, 0], slice_points[:, 1]), slice_values,
                                      (grid_p1, grid_p2), method='linear')

                    surf = ax.plot_surface(grid_p1, grid_p2, grid_nmb, cmap='RdYlGn', alpha=0.8)
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/three_way_dsa_3d_{model_name.lower().replace(' ', '_')}.png", bbox_inches='tight')
        plt.savefig(f"{output_dir}/three_way_dsa_3d_{model_name.lower().replace(' ', '_')}.pdf", bbox_inches='tight')
        plt.close()
...
class DCEDataProcessor:
    ...
    def load_choice_data(self, 
                         ...
                         task_col: str = 'choice_task') -> pd.DataFrame:
        """
        ...
        missing_cols = [col for col in required_cols if col not in self.choice_data.columns]
        if missing_cols:
    ...
    def _validate_choice_data(self, choice_col: str, id_col: str, task_col: str):
        ...
        if not all(grouped == 1):
            ...
            raise ValueError(f"Each choice task should have exactly one selected alternative. "
...
class DCEAnalyzer:
    ...
    def _calculate_simple_attribute_weights(self, 
        ...
        for attr in attributes:
            ...
            choice_by_attr = data.groupby(attr)[choice_col].agg(['mean', 'count']).reset_index()
            # This is a very simplified approach - in reality you'd use proper MLE
            correlation = np.corrcoef(choice_by_attr[attr], choice_by_attr['mean'])[0, 1]
            
            results['estimated_coefficients'][attr] = correlation
            results['standard_errors'][attr] = 0.1  # Placeholder
            results['p_values'][attr] = 0.05 if abs(correlation) > 0.1 else 0.5  # Placeholder
    ...
    def analyze_stakeholder_preferences(self, 
        ...
        if attribute_importance and self.model_results.get('estimated_coefficients'):
            ...
            if abs_coeffs:
                ...
                if total_importance > 0:
                    ...
                    results['attribute_importance'] = {k: 0 for k in abs_coeffs.keys()}
        ...
        if willingness_to_pay:
            ...
            for attr in self.model_results.get('attributes', []):
                if 'cost' in attr.lower() or 'price' in attr.lower():
                    cost_attr = attr
                    break
...
def design_dce_study(attributes: Dict[str, Dict],
                    num_choices: int = 12,
                    alternatives_per_task: int = 2,
                    num_repeats: int = 2) -> pd.DataFrame:
    """
...
def integrate_dce_with_cea(dce_results: Dict, cea_results: Dict) -> Dict:
    ...
    if 'attribute_importance' in dce_results:
        stakeholder_values = dce_results['attribute_importance']
    ...
    integrated_results['integrated_analysis'] = {
        ...
        'dce_alignment_score': np.random.uniform(0.5, 1.0),  # Placeholder for actual alignment metric
        'value_components': {
            'health_system': cea_results.get('incremental_nmb', 0),
            'societal': cea_results.get('incremental_nmb', 0),  # Would be different in full implementation
    ...
    integrated_results['policy_recommendations'] = {
        ...
        'evidence_gaps': ['Stakeholder preference validation', 'Real-world implementation feasibility'],
        'priority_actions': ['Conduct full-scale DCE', 'Validate with real-world evidence']
    }
    
    return integrated_results


def calculate_analytical_capacity_costs(dce_study_size: int, 
    ...
    costs = {
        ...
        'implementation': {
            'process_adaptation': 200000 if country == 'NZ' else 180000,  # For HTA agencies
            'staff_training': 30000 if country == 'NZ' else 25000,
        }
    }
    
    total_cost = (sum(costs['study_design'].values()) + 
    ...
    results = {
        ...
        'cost_per_stakeholder_group': total_cost / len(stakeholder_groups) if stakeholder_groups else 0,
...
if __name__ == '__main__':
    ...
    design = design_dce_study(dce_attributes, num_choices=12, alternatives_per_task=2)
    print(f"   Created {len(design)} alternatives across {design['choice_task'].nunique()} choice tasks")
    ...
    for resp_id in range(1, n_respondents + 1):
        for task_id in range(1, n_tasks + 1):
            ...
            for idx, row in task_alts.iterrows():
                ...
                utility = (0.0001 * row['cost_per_qaly'] * -1 +  # Cost coefficient
                          (1 if row['perspective'] == 'societal' else 0) * 0.5 +  # Societal preference
                          (1 if row['intervention_type'] == 'preventive' else 0) * 0.3 +  # Prevention preference
                          np.random.gumbel(0, 1))  # Add random error term
                
                # Determine choice based on utility (highest utility selected)
                if idx == task_alts.index[0]:  # Initialize max utility for this task
                    best_idx = idx
                    max_utility = utility
            ...
            for idx, row in task_alts.iterrows():
                ...
                # Add the attributes
                for attr in ['perspective', 'cost_per_qaly', 'population_size', 'intervention_type']:
    ...
    df_synthetic.to_csv('/Users/doughnut/Library/CloudStorage/OneDrive-VictoriaUniversityofWellington-STAFF/Submitted/policy_societaldam_pharma/canonical_code/data/synthetic_dce_data.csv', index=False)
    print(f"   Created synthetic choice data for {n_respondents} respondents across {n_tasks} tasks")
    
    # Process the data
    print("\n3. Processing choice data...")
    processor = DCEDataProcessor()
    processor.define_attributes(dce_attributes)
    ...
    print(f"   Integration completed: CEA favorable = {integrated_results['integrated_analysis']['cea_favorable']}")

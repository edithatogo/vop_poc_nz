...
def generate_policy_brief(
    ...
    for name, res in intervention_results.items():
        ...
        soc_icer = res["societal"]["human_capital"]["icer"] # Default to human capital for summary
        
        hs_decision = "FUND" if res["health_system"]["is_cost_effective"] else "REJECT"
        soc_decision = "FUND" if res["societal"]["human_capital"]["is_cost_effective"] else "REJECT"
    ...
    lines.append("Adopting a societal perspective reveals significant opportunity costs (benefits lost) when decisions are restricted to health system costs only.")
    lines.append("\n![Discordance Loss](../figures/discordance_loss.png)")
    
    lines.append("\n## 3. Equity-Efficiency Trade-offs")
    lines.append("Does the societal perspective sacrifice equity? The chart below shows the relationship between total efficiency gains and equity improvements.")
    ...
    lines.append("> **Policy Action:** Review interventions where Societal Decision differs from Health System Decision, especially where Equity Impact is Positive.")
...
if __name__ == "__main__":
    ...
    dummy_results = {
        "Test Intervention": {
            ...
            "dcea_equity_analysis": {"weighted_total_health_gain": 120, "total_health_gain": 100}
        }
    }
